//! Optimized SIMD NTT using register-optimized operations
    ...
    unsafe fn forward_simd(&self, a: &mut [i32; 256]) {
        ...
        while len >= 2 {
            for start in (0..N).step_by(2 * len) {
                ...
                // processing 8 elements per time for simd
                while j + 8 <= end {
                    let a_lower = [
                        a[j],
                        a[j + 1],
...
mod tests {
    ...
    fn compare_with_basic() {
        ...
        opt_ntt.forward(&mut poly2);
        assert_eq!(poly1, poly2);

        basic_ntt.inverse(&mut poly1);
        opt_ntt.inverse(&mut poly2);
        assert_eq!(poly1, poly2);
    }
}
